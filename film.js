...
const detailyFilmu = document.querySelector('#detail-filmu')
const nazvy = document.querySelector('.card-title')
const texty = document.querySelector('.card-text')
const obrazek = document.querySelector('.img-fluid')

let hash = location.hash.substring(1)
const indexy = (film) => film.id === hash;
const haha = filmy.find(indexy)
nazvy.textContent = haha.nazev;
texty.textContent = haha.popis;
obrazek.src = haha.plakat.url;


//6
const primeTime = document.querySelector('#premiera')
const date = dayjs(haha.premiera).format('DD.MM.YYYY')
const dny = dayjs(haha.premiera).diff(dayjs(),'days')

const sklon = (pocet) => {
	if (pocet === 1) {
		return "den";}
		else if (pocet >= 2 && pocet <=4) {return "dny";}
		else {return "dnů";}}
		
primeTime.innerHTML = `Premiéra <strong> ${date}</strong>, což je za ${dny} ${sklon(dny)}.`
...
hvezdy.forEach((hvezda) => {hvezda.addEventListener('click', (e) => {pocty =
